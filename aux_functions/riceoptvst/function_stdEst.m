function sigma_hat=function_stdEst(z,kernel_type,scalar,est_type,which_dims,mask)
...
if est_type==1        %%% median of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat=median(abs(z(mask==1)))/0.674489750196082;   %  assumes, for simplicity, that median(z(:))=0.
    else
...
elseif est_type==-1   %%% median of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat=median(abs(z(mask==1)-median(z(mask==1))))/0.674489750196082;  % 0.674489750196082=icdf('normal',3/4,0,1) 
    else
...
elseif est_type==2    %%% mean of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat=mean(abs(z(mask==1)))*sqrt(pi/2);   %  assumes, for simplicity, that mean(z(:))=0.
    else
...
elseif est_type==-2    %%% mean of absolute deviations  
    ...
    if scalar==1
        ...
        sigma_hat=mean(abs(z(mask==1)-mean(z(mask==1))))*sqrt(pi/2);
    else
...
elseif est_type==3     %%% sample standard deviation
    ...
    if scalar==1
        ...
        sigma_hat=sqrt(mean(abs(z(mask==1)).^2));    %  assumes, for simplicity, that mean(z(:))=0.
    else
...
elseif est_type==-3    %%% sample standard deviation
    ...
    if scalar==1
        ...
        sigma_hat=sqrt(mean(abs(z(mask==1)-mean(z(mask==1))).^2));
    else
